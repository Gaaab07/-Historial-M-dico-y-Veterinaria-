...
namespace COMPLETE_FLAT_UI
{
    public partial class FormMenuPrincipal : Form
    {
        ...
        private void btnListaClientes_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                //Integrated Security=SSPI;Persist Security Info=False;Initial Catalog=VeterinariaDB;Data Source=DESKTOP-HTCT9FE

                // Cadena de conexión
                using (SqlConnection conexionn = new SqlConnection("Integrated Security=SSPI;Persist Security Info=False;Initial Catalog=VeterinariaDB;Data Source=DESKTOP-HTCT9FE"))
                {
                    string query = "SELECT * FROM CLIENTE";
                    SqlDataAdapter da = new SqlDataAdapter(query, conexionn);
                    DataTable dt = new DataTable();
                    da.Fill(dt);

                    // Asignar el DataTable al DataGridView usando el método SetDataSource
                    fm.SetDataSource(dt);
                }
            }
            catch (Exception ex)
        ...
        {
            FromMascota frm = new FromMascota();
        ...
        {
            FormTratamiento frm = new FormTratamiento();
            frm.FormClosed += new FormClosedEventHandler(MostrarFormLogoAlCerrarForms);
            AbrirFormEnPanel(frm);
        }
